...
pub fn get_connection(worker_type: &str, options: &Opt) -> Result<WorkerConnection, String> {
    ...
    let connection_future = match &options.command {
        Command::Receptionist {host, port, connect_with_external_ip } => {
        ...
        Command::Locator {token, project_name} => {
            let params = ConnectionParameters::new(worker_type)
                .using_raknet()
                .using_external_ip(true)
                .enable_internal_serialization();

            let locator_params = LocatorParameters::new(project_name, LocatorCredentials::login_token(token));
            ...
            WorkerConnection::connect_locator_async(&locator, deployment, &params, queue_status_callback)
        }
    };
